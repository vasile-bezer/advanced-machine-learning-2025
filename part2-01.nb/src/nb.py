...
def nb(X, labels):
	...
	theta_one = X[labels == 1].mean(axis = 0)
	theta_zero = X[labels == 0].mean(axis = 0)
	p_pos = np.sum(labels == 1)
	p_neg = np.sum(labels == 0)
	w = np.log(theta_one / theta_zero) - np.log((1 - theta_one) / (1 - theta_zero))
	b = np.log(p_pos / p_neg) + np.sum(np.log((1 - theta_one) / (1 - theta_zero)))
	return w, b
