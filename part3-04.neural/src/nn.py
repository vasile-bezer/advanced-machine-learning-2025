...
class Neuron:
	...
	def compute_output(self):
		...
		# place your code here
		self.input = np.sum(e.w * e.inn.output for e in self.inedges) + self.bias
	...
	def compute_delta(self):
		...
		# compute self.delta
		# place your code here
		self.delta = self.der(self.input) * np.sum(e.w * e.outn.delta for e in self.outedges)
...
class NN:
	...
	def forward(self, x):
		...
		# place your code here
		for neuron in self.neurons[cnt:]:
	...
	def gradients(self, X, y):
		...
		for i in range(cnt):
			...
			for j, e in enumerate(self.edges):
				wgrad[j] += e.inn.output * e.outn.delta
