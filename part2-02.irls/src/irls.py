...
def irls(X, labels, w, itercnt):
	...
	z = X.dot(w)
	mu = 1 / (1 + np.exp(-z))
	err[0] = -np.sum(labels * np.log(mu + 1e-15) + (1 - labels) * np.log(1 - mu + 1e-15))
	misclass[0] = np.mean((mu >= 0.5) != labels)# / np.size(y, axis=0)
	for i in range(itercnt):
		...
		z = X.dot(w)
		mu = 1 / (1 + np.exp(-z))
		err[i+1] = -np.sum(labels * np.log(mu + 1e-15) + (1 - labels) * np.log(1 - mu + 1e-15))
		misclass[i+1] = np.mean((mu >= 0.5) != labels)
