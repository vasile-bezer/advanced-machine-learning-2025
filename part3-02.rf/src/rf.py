...
def fit(X, y, featurecnt):
	...
	selected_features = np.random.choice(feasible, min(len(feasible), featurecnt), replace=False)
	g_selected = g[selected_features]
	ind = selected_features[np.argmax(g_selected)]
...
def rf(Xtrain, ltrain, treecnt, samplecnt, featurecnt, Xtest, ltest):
	...
	for t in range(treecnt):
		sample_indices = np.random.choice(cnt, samplecnt, replace=True)
		Xtrain_sampled = Xtrain[sample_indices]
		ltrain_sampled = ltrain[sample_indices]
		
		tree = fit(Xtrain_sampled, ltrain_sampled, featurecnt)
		
		p += predict(tree, Xtest)
		
		misclass[t] = np.mean(np.sign(p) != ltest)
